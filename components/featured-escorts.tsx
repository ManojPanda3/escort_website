...
function FeaturedEscort({ username, profile_picture, age, location_name, price, rank }: FeaturedEscortProps) {
  const [isHovered, setIsHovered] = useState(false)

  return (
    <motion.div
      className="relative w-full h-[500px] rounded-xl overflow-hidden cursor-pointer"
      ...
      <Card className="w-full h-full bg-black/40 backdrop-blur-sm overflow-hidden">
        <div className="relative w-full h-full">
          <Image
            ...
            priority
          />
          <div className={`absolute inset-0 bg-gradient-to-t ${rankColors[rank]} opacity-50`}></div>
          <div className="absolute inset-0 border-4 border-gold rounded-xl pointer-events-none"></div>
          ...
          <AnimatePresence>
            {isHovered ? (
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <h3 className="text-3xl font-bold mb-4 text-white">{username}</h3>
                <p className="text-lg text-center mb-4 text-gray-300">
                  {age} years | {location_name}
                </p>
                <p className="text-2xl font-bold mb-6 text-amber-400">{price}</p>
                <div
                  className={`px-4 py-2 rounded-full bg-gradient-to-r ${rankColors[rank]} text-white font-bold flex items-center`}
                >
                  <Star className="w-5 h-5 mr-2" />
                  <span>Escort of the {rank.charAt(0).toUpperCase() + rank.slice(1)}</span>
                </div>
                <motion.div
              ...
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <h3 className="text-3xl font-bold mb-2 text-white">{username}</h3>
...
export function FeaturedEscorts({ users }: { users: FeaturedEscortProps[] }) {
  ...
  }, [users.length]) // Added nextEscort and users.length to dependencies
  ...
  return (
    ...
    <h2 className="text-4xl font-extrabold text-center mb-12 bg-gradient-to-r from-amber-200 to-yellow-400 bg-clip-text text-transparent font-playfair-display">
      ...
      <div className="flex justify-center items-center sm:gap-4 overflow-x-auto sm:overflow-x-visible px-4 sm:px-0">
