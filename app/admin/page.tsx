...
const getData = unstable_cache(
  async (date) => {
    const [transactionsResponse, usersResponse, ageProofResponse, offersResponse, locationsResponse] = await Promise.all([
      supabaseAdmin
        .from('transactions')
...
export default async function AdminPage() {
  ...
  // Date 1 year ago
  const date = new Date(new Date().setFullYear(new Date().getFullYear() - 1)).toISOString();

  // Fetch data from cache or database
  const { transactions, transactionError, users, usersError, ageProof, ageProofError, offers, offersError, locations, locationsError } = await getData(date);

  // Handle errors
  if (transactionError || usersError || ageProofError || offersError || locationsError) {
    console.error('Error fetching data:', { transactionError, usersError, ageProofError, offersError, locationsError });
