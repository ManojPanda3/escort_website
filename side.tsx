...
import { X, Check, Settings, DollarSign, MessageSquare } from "lucide-react"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Badge } from "@/components/ui/badge"
import { StoryCircle } from "./story-circle"
import { Star, MapPinIcon, ClockIcon, UserIcon, RulerIcon, CheckCircleIcon, CheckIcon } from "lucide-react"
                          ...
                          <div className="flex items-center gap-4 relative h-full bg-gray-400 outline outline-1 outline-white rounded-sm">
                            ...
                            {isExpanded && (
                              <motion.div
                                ...
                                className="w-full max-w-md mx-auto bg-background mt-4 rounded-lg shadow-lg overflow-hidden"
                              >
                                <div className="p-6 space-y-6">
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {["Fashion", "Travel", "Food", "Music", "Art", "Sports"].map(
                                        (interest, index) => (
                                          <div
                                            key={index}
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div className="grid gap-3">
                                      {rates.map((rate) => (
                                        ...
                                        >
                                          <div>
                                            <p className="font-medium">{rate.reason}</p>
                                            <p className="text-sm text-muted-foreground">{rate.duration}</p>
                                          </div>
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div className="flex items-center gap-2 text-muted-foreground">
                                      ...
                                      <span>{currentEscort.location_name}</span>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex justify-center gap-4 pt-4">
                                    <Button
                                      variant="outline"
                                      size="lg"
                                      className="rounded-full w-16 h-16 bg-red-100 hover:bg-red-200 border-red-200"
                                      onClick={() => {
                                        /* Handle reject */
                                      }}
                                    >
            ...
            <AnimatePresence>
              {/* {isExpanded && (
                <motion.div
                  ...
                  animate={{ scaleY: 1, opacity: 1 }}
                  exit={{ scaleY: 0, opacity: 0 }}
                  className="w-full h-screen bg-white dark:bg-gray-900 mt-4 p-6 rounded-lg relative overflow-y-auto"
                >
                  ...
                  <Tabs defaultValue="pictures" className="space-y-4">
                    <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 gap-2">
                      <TabsTrigger value="pictures" className="flex items-center gap-2 text-xs sm:text-sm">
                        <Image
                          src="/placeholder.svg"
                          alt="Pictures"
                          width={16}
                      ...
                      <TabsTrigger value="rates" className="flex items-center gap-2 text-xs sm:text-sm">
                        <DollarSign className="h-4 w-4 hidden sm:block" />
                        Rates
                      </TabsTrigger>
                      <TabsTrigger value="testimonials" className="flex items-center gap-2 text-xs sm:text-sm">
                    ...
                    <TabsContent value="pictures">
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {pictures.map((pic) => (
                          <Card key={pic.created_at} className="overflow-hidden hover:shadow-lg transition-shadow">
                            ...
                            <CardContent className="p-4">
                              <p className="font-medium truncate">{pic.title}</p>
                            </CardContent>
                          </Card>
                        ))}
                      </div>
                        ...
                        <CardContent className="p-6">
                          <div className="grid gap-4">
                            {services.map((service) => (
                              <div
                                key={service.id}
                                className="flex items-center justify-between gap-2 text-sm p-3 border rounded-md"
                              >
                            ...
                            <CardContent className="p-6">
                              <div className="flex items-start gap-4">
                                ...
                                <div className="min-w-0">
                                  <h3 className="font-semibold truncate">{testimonial.users?.name || "Anonymous"}</h3>
